...
- [修复] PE/PB 批量拉取：`_fetch_valuation_bulk` 新增腾讯行情接口作为 efinance 失败后的 fallback，解决 VPS 被墙时 efinance 不可用导致全市场 PE 全部缺失、PE 过滤失效的问题；`DatabaseManager` 新增 `get_market_cache_codes()` 支持按日期批量查股票列表
- [改进] RS均线趋势：个股强弱改以"股价/指数收盘比值"时间序列的 MA5/MA10 方向为主判据（RS线上行=强势/下行=弱势/横盘=中性细分），替代原三截面点位逻辑，避免强弩之末被误判为强势
- [改进] 大盘环境调权：个股分析新增大盘技术状态前置判断，大盘空头/震荡时按比例降低个股信号评分并重新判定买入信号，大盘多头时不干预
- [改进] 大盘复盘数据有效性卫兵：涨跌家数与成交额均为零时（非交易日或数据源全部失败），跳过复盘报告生成并返回 None，避免 LLM 基于空数据生成误导性分析
